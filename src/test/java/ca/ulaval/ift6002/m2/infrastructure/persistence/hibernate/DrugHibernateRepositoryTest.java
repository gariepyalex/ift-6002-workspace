...
public class DrugHibernateRepositoryTest {
	...
	private static final DrugDTO TYLENOL_DTO = new DrugDTO(
			TYLENOL_DIN.getValue(), TYLENOL_BRAND_NAME, TYLENOL_DESCRIPTOR);
	...
	public void setUp() {
		willReturn(entityManager).given(entityManagerProvider)
				.getEntityManager();
		drugRepository = new DrugHibernateRepository(entityManagerProvider,
				drugDTOAssembler);
	}
	...
	public void whenFindByBrandNameOrDescriptionShouldVerifyFromDTOsAssemblerCall() {
		...
		verify(drugDTOAssembler, times(1)).fromDTOs(
				anyCollectionOf(DrugDTO.class));
	}

	private void setUpEntityManagerWithTylenol() {
		willReturn(TYLENOL_DTO).given(entityManager).find(DrugDTO.class,
				TYLENOL_DIN.getValue());
	}
